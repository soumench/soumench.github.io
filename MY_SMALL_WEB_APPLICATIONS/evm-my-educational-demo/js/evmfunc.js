...
( Strictly for Educational Demonstration, Non-Profit and Strictly Non-Commercial purpose. All Rights Reserved. )*/
    function evmOn(){longBeepsound.play();//playbeep();
                lampon(LPon); On.value="ON"; candLampsOff(); evmONTog = true; displayColor("aqua");
                if(pollIsClosed){display(closedMsg);}else display(onMsg);
            }
    function evmOff(){
        evmONTog=false; allLamsOff(); On.value="OFF"; display(""); displayColor("#000000");
        shortBeepSound.play();//playpNeek();
    }        
    ...
    function pollClosed(){LcandOFF(msgDuration); pollIsClosed=true; display(closedMsg);return;}        
    function BallotIssuedFunc(){//playswitchsound();
    ...
    function addvote(s){longBeepsound.play();// long beep
    ...
    function candLampsOff(){
        paperTrailPos("100px");
        paperTrail.style.zIndex=-2;
    ...
    function candBtnAction(){ ballotIssued = false; LcandOFF(msgDuration); //playswitch();
    }
    ...
    function paperTrailAnimAppear(){vvpatSound.play();//add sound here
